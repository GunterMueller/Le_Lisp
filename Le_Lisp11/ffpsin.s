...
* ?          opt       pcs
 ...
 entry      ffpsin,ffpcos,ffptan,ffpsincs entry points
 
 extern      ffptheta                    inverse tangent table
 
 extern      ffpmul,ffpdiv,ffpsub    multiply, divide and subtract
...
********************************************
ffpsincs move.w    #-2,-(a7)           flag both sine and cosine wanted
         bra.s     fpscom              enter common code
 
**********************
* tangent entry point*
**********************
ffptan   move.w    #-1,-(a7)           flag tangent with minus value
         bra.s     fpschl              check very small values
 
**************************
* cosine only entry point*
**************************
ffpcos   move.w    #1,-(a7)            flag cosine with positive value
         bra.s     fpscom              enter common code
 
* negative sine/tangent small value check
fpschm   cmpi.b     #$80+$40-8,d7       Q less or same as 0-2**-9
         bhi.s     fpscom              continue if not too small
* return argument
fpsrti   adda.l     #2,a7               rid internal parameter
...
*        or.b      #$02,ccr            force v bit on
         ...
         movem.l   (a7)+,d1-d6/a0      restore registers
         adda.l     #2,a7               clean internal argument off stack
         rts                           return to caller
...
* convert quotient to float integer
         ...
         ori.b      #$ff,d4             do not remove sign and exponent
...
fpsnck   cmp.l     d4,d7               Q less than -pi
...
fpsckm   move.l    #0,d5               x=0
         ...
         move.l    #fixedpi>>2&$3fffffff,d4      setup fixed pi/2 constant
...
* cordic loop
fsinlp   addi.w     #1,d2               increment shift count
         move.l    d5,d3               copy x
         move.l    d6,d4               copy y
...
fscom    move.w    7*4(a7),d1          reload internal parameter
         bpl.s     fssincos            branch for sine or cosine
 
         addi.b     #1,d1               see if was 0-1 for tangent
...
fsinfrt  movem.l   (a7)+,d1-d6/a0      restore registers
         adda.l     #2,a7               delete internal parameter
         rts                           return to caller
...
fsdual   move.l    d5,-(a7)            save cosine derivitive
         bsr.s     fsfloat             convert sine derivitive to float
         move.l    d6,6*4(a7)          place sine into saved d6
         move.l    (a7)+,d6            restore cosine derivitive
...
fsfloat  move.l    d6,d4               copy internal precision value
         ...
         cmpi.l     #$000000ff,d6       Q test magnitude
	 bhi.s     ffptno              normalize if not too small
fsfzro   move.l    #0,d6               return a zero
         rts                           return to caller
fsfneg   asr.l     #8,d4               see if all ones bits 8-31
         addi.l     #1,d4               Q goes to zero
	 bne.s     ffptno              normalize if not too small
