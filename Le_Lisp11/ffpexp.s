...
* ?          opt       pcs
 ...
 entry      ffpexp                        entry point
 
 extern      ffphthet                    hypertangent table
 
 extern      ffpmul,ffpsub    arithmetic primitives
...
fpeovflw move.w    (a7)+,d6            load sign word and work off stack
         tst.b     d6                  Q was argument negative
...
fpovrtn  movem.l   (a7)+,d1-d6/a0      restore registers
         rts                           return to caller
 
* return one for zero argument
ffpe1    move.l    #$80000041,d7       return a true one
         lea       7*4+2(a7),a7        ignore stack saves
...
ffpexp   movem.l   d1-d6/a0,-(a7)      save all work registers
         move.w    d7,-(a7)            save sign in low order byte for later
...
fpepos   move.l    d7,d2               save original argument
	 ...
	 jsr       ffpmul              obtain division thru multiply
...
* convert quotient to both fixed and float integer
         ...
         subi.b     #64+32,d5           find non-fractional precision
         neg.b     d5                  make positive
         cmpi.b     #24,d5              Q insure not too large
         ble.s     fpeovflw            branch too large
         cmpi.b     #32,d5              Q test upper range
         bge.s     fpesml              branch less than one
         lsr.l     d5,d7               shift to integer
         move.b    d7,(a7)             place adjusted exponent with sign byte
...
*************************
cordic   addi.w     #1,d2               increment shift count
         move.l    d5,d3               copy y
         move.l    d6,d4               copy x
